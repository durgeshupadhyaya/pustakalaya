                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($orders as $key => $order)
                            <tr>
                                ...
                                <td>
                                    <a title="View" href="{{ route('admin.orders.items', $order->id) }}"
                                        style="float: left;margin-right: 5px;" class="btn btn-sm btn-outline-primary"><i
                                            class="fa-solid fa fa-eye"></i></a>

                                    <form class="delete-form" action="{{ route('admin.orders.delete', $order->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-danger delete_order mr-2"
